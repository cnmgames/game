...
(function(){
  ...
  function init(){if(isActive()){hideGate();return;}var ut=localStorage.getItem("user_token");if(ut){fetch("https://k.ttla.top/api.php?action=user/info",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:ut})}).then(function(r){return r.json();}).then(function(d){if(d.success&&d.code_info&&d.code_info.status==="used"){hideGate();}else{window.location.href="/user";}}).catch(function(){window.location.href="/user";});}else{window.location.href="/login";}}
