...
export function HomeView({ players, themes, onSelectTheme, onStartGame }: HomeViewProps) {
  return (
    <div className="flex flex-col gap-6 py-4">
      ...
      <div className="flex flex-col gap-3">
        {players.map((player, idx) => {
          ...
          return (
            <div
              key={player.id}
              className="ios-card p-4 flex items-center justify-between ios-btn cursor-pointer border border-white/10 active:scale-[0.98] transition-transform"
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-sm font-medium text-gray-400 mt-0.5 truncate">
                    {theme?.name || '未选择主题'}
                  </div>
                </div>
              </div>
      ...
      <button
        className="w-full h-12 bg-white rounded-full text-black font-semibold text-base shadow-lg ios-btn flex items-center justify-center gap-2 mt-4 active:scale-[0.98] transition-transform"
        onClick={onStartGame}
      >
        <span>开始游戏</span>
